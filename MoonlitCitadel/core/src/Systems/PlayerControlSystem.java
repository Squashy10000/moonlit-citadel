...
public class PlayerControlSystem extends IteratingSystem {
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        ...
        if (gameInput.isLeft()){
            bodyComponent.getBody().setLinearVelocity(-2.5f, 0f);
            bodyComponent.getBody().setAngularVelocity(0f);
        ...
        if (gameInput.isUp()){
            bodyComponent.getBody().setLinearVelocity(0f, 2.5f);
            bodyComponent.getBody().setAngularVelocity(0f);
        ...
        if (!gameInput.isLeft()&&!gameInput.isDown()&&!gameInput.isRight()&&!gameInput.isUp()){
            bodyComponent.getBody().setLinearVelocity(0f, 0f);
            bodyComponent.getBody().setAngularVelocity(0f);
